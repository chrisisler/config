...
var makeFunctionParse = _interopDefault(require('parse-function'));
...
function ruleIsWildcard(rule) {
  return rule.allReflectedArgs.some(arg => arg.argName === '_' && !('pattern' in arg) && !('subPatterns' in arg) && arg.isDestructured === false) && rule.arity === 1;
}
...
function ruleMatchesNumberInput(numberInput, pattern, rules, inputIndex) {
...
function ruleMatchesObjectInput(objectInput, inputIndex, rules, ruleIndex, objectPattern) {
  ...
  const argNameMatchesProp = () => {
    return desiredKeys.some((inputKey, keyIndex) => {
      ...
      const doesMatch = isPatternAcceptable(rules, ruleIndex, keyIndex, objectInputValue, reflectedArg);
 ...
 */
function getParentClassName(instance) {
  if (instance == null) {
    return;
  }
...
reflectedArg) {
  ...
  //   (x = Person) => 'awesome'
  // )
  const hasCustomTypes = 'customTypeNames' in reflectedArg && Array.isArray(reflectedArg.customTypeNames);
  if (hasCustomTypes) {
    const inputTypeName = input.constructor.name;
    ...
    const parentClassName = getParentClassName(input);
    // console.log('parentClassName is:', parentClassName)

    if (parentClassName != null && reflectedArg.customTypeNames != null && reflectedArg.customTypeNames.includes(parentClassName)) {
      return true;
    } else if (reflectedArg.pattern != null && reflectedArg.pattern.toString() !== inputTypeName) {
      // If another rule has a user-defined data type that matches, don't throw
      const otherRuleMatchesUserDefinedType = rules.filter((_, i) => i !== ruleIndex).some(rule => rule.allReflectedArgs.some(({ customTypeNames }) => Array.isArray(customTypeNames) && customTypeNames.includes(inputTypeName)));
  ...
  if (!TYPES.includes(pattern)) {
    ...
    if (isType('Promise', pattern) && typeof pattern.then === 'function') {
      ...
      const guardResult = predicate(input);
  ...
  let numberMatch = ruleMatchesNumberInput(input, pattern, rules, inputIndex);
  if (numberMatch) {
    return true;
  }

  // getType(input).includes('Function') ???
  if (isType('Function', input) || isType('GeneratorFunction', input) || isType('AsyncFunction', input)) {
    if (Function === pattern) {
      return true;
    }
  }
...
function toString(x) {
  // prettier-ignore
  return isFunction(x) ? String(x) : Array.isArray(x) ? x.map(toString) : JSON.stringify(x, null, 2);
}

function wavematch(...inputs) {
  invariant(inputs.length === 0, 'Please supply at least one argument. Cannot match on zero parameters.');
  ...
  return function (...rawRules) {
    ...
    const key = JSON.stringify(inputs.map(toString), null, 2) + JSON.stringify(rawRules.map(String), null, 2);
    ...
    const indexOfWildcardRule = rules.findIndex(rule => rule.allReflectedArgs.some(reflectedArg => {
...
wavematch.create = (...rawRules) => (...inputs) => wavematch(...inputs)(...rawRules);
